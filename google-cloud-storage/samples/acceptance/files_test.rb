...
require_relative "../storage_initiate_resumable_upload"
...
describe "Files Snippets" do
  ...
  it "starts resumable upload" do
    file_content = "A" * (3 * 1024 * 1024) # 3 MB of 'A' characters
    file = StringIO.new file_content
    chunk_size = 256 * 1024 * 3 

    assert_output "Resumable upload completed for #{remote_file_name}\n" do
      initiate_resumable_upload bucket_name: bucket.name, file: file, file_name: remote_file_name, chunk_size: chunk_size
    end
    assert_equal bucket.files.first.name, remote_file_name
  end

  # it "Delete resumable upload" do
  #   file_content = "A" * (3 * 1024 * 1024) # 3 MB of 'A' characters
  #   file = StringIO.new file_content
  #   chunk_size = 256 * 1024 * 3 
  ...
  #   assert_not_equal bucket.files.first.name, remote_file_name
  # end

  # it "restarts a resumable upload" do
  #   file_content = "A" * (3 * 1024 * 1024) # 3 MB of 'A' characters
  #   file = StringIO.new file_content
  #   chunk_size = 256 * 1024 * 3 
  #   initiate_resumable_upload bucket_name: bucket.name, file: file, file_name: remote_file_name, chunk_size: chunk_size
