...
describe Google::Cloud::Storage::Bucket, :mock_storage do
  ...
  it "creates a file with checksum: :crc32c by default" do
    ...
    content = "Hello world!"
      ...
      mock.expect :insert_object, create_file_gapi(bucket.name, new_file_name),
        [bucket.name, empty_file_gapi(crc32c: crc32c, content: content)], **insert_object_args(name: new_file_name, upload_source: tmpfile, options: {retries: 0})
  ...
  it "creates a file with crc32c if checksum is true" do
    ...
    Tempfile.open ["google-cloud", ".txt"] do |tmpfile|
      ...
      mock = Minitest::Mock.new
      mock.expect :insert_object, create_file_gapi(bucket.name, new_file_name),
        [bucket.name, empty_file_gapi(content: content, checksum: true, crc32c: "e5jnUQ==")], **insert_object_args(name: new_file_name, upload_source: tmpfile, options: {retries: 0})
      bucket.service.mocked_service = mock
